...
function validate(form) {
  const errs = {};
  if (!form.name) errs.name = "Le nom est requis.";
  if (!form.email) {
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (typeof auth?.register !== "function") throw new Error("AuthContext non disponible.");
      ...
      if (result?.success) {
        ...
        setServerErr(result?.message || "Une erreur est survenue.");
      }
    } catch (err) {
      setServerErr(err?.message || "Erreur inattendue.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="auth-form-panel">
        ...
        <div className="form-card">
          ...
          <div className="form-header">
            <div className="page-label"><MdPersonAdd size={12}/> Nouveau Compte</div>
          ...
          {success && (
            <div className="success-banner">
              <MdCheckCircle size={18}/> Compte créé ! Redirection vers la connexion…
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="r-name">Nom complet</label>
              <div className="input-wrap">
                <input id="r-name" className={`form-input${errors.name ? " error-input" : ""}`}
                  type="text" name="name" placeholder="Votre nom complet"
                  autoComplete="name" value={form.name} onChange={handleChange}/>
              </div>
              {errors.name && <div className="field-error"><MdError size={13}/> {errors.name}</div>}
            </div>

            {/* Email */}
            <div className="form-group">
              <label className="form-label" htmlFor="r-email">Adresse Gmail</label>
              <div className="input-wrap">
                <input id="r-email" className={`form-input${errors.email ? " error-input" : ""}`}
            ...
            {/* Password */}
            <div className="form-group">
              <label className="form-label" htmlFor="r-pass">Mot de passe</label>
              <div className="input-wrap">
                <input id="r-pass" className={`form-input has-eye${errors.password ? " error-input" : ""}`}
            ...
            <div className="form-group">
              ...
              <div className="input-wrap">
                <input id="r-conf" className={`form-input has-eye${errors.confirm ? " error-input" : ""}`}
              ...
              {errors.confirm && <div className="field-error"><MdError size={13}/> {errors.confirm}</div>}
            </div>

            <button type="submit" className="btn-submit" disabled={loading || success}>
              {loading ? <><span className="btn-spinner"/> Création en cours…</>
